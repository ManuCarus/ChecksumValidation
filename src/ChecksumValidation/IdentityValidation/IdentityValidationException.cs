...
{
    public sealed class IdentityValidationException : ChecksumException
    {
        // private members
        ...
        // ctors
        public IdentityValidationException(string code, string message, string identityType, string identityId) : base(code, message)
        {
            ...
            if (String.IsNullOrEmpty(identityType)) throw new NullReferenceException("identity type must not be empty");
            if (String.IsNullOrEmpty(identityId)) throw new NullReferenceException("identity id must not be empty");

            this.identityType = identityType;
            this.identityId = identityId;
        }

        internal IdentityValidationException(string code, string message, string identityType, string identityId, Exception ex) : base(code, message, ex)
        {
            if (String.IsNullOrEmpty(code)) throw new NullReferenceException("code must not be empty");
            if (String.IsNullOrEmpty(message)) throw new NullReferenceException("message must not be empty");
            if (String.IsNullOrEmpty(identityType)) throw new NullReferenceException("identity type must not be empty");
